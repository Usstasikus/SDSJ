...
def inventory_data_nodir(input_dir):
    ...
    training_names = ls(os.path.join(input_dir, '*/train.csv'))
    for i in range(0,len(training_names)):
        name = training_names[i]
        training_names[i] = name.split(os.sep)[-2]
...
def inventory_data_dir(input_dir):
    ...
    training_names = ls(input_dir + '/*_train.csv') # This supports subdirectory structures obtained by concatenating bundles
    for i in range(0,len(training_names)):
        name = training_names[i]
...
def check_dataset(dirname, name):
    ...
    #     exit(1)
    test_file = os.path.join(dirname, name, 'test.csv')
