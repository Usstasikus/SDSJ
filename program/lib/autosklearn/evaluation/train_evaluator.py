...
class TrainEvaluator(AbstractEvaluator):
    ...
    def fit_predict_and_loss(self, max_iter=0):
        if max_iter != 0:
            if self.cv_folds > 1:
    ...
    def partial_fit_predict_and_loss(self, fold, max_iter=0):
    ...
    def _partial_fit_and_predict(self, fold, train_indices, test_indices,
        ...
        if max_iter != 0:
            ...
            # Do only output the files in the case of max_iter holdout,
            # In case of max_iter partial cv, no file output is needed
            ...
            if model.estimator_supports_iterative_fit():
                ...
                while (
                    ...
                    and (max_iter == -1 or max_iter > total_n_iteration)
                ):
                    ...
                    if (
                        ...
                        or (max_iter > 0 and total_n_iteration >= max_iter)
                    ):
                        final_call = True
                    else:
                        final_call = False
    ...
    def get_splitter(self, D):
        ...
        if D.info['task'] in CLASSIFICATION_TASKS and \
            ...
            if self.resampling_strategy in ['holdout',
                ...
                # Figure out which sklearn method would do best here
                cv = ShuffleSplit(n_splits=1, train_size=train_size,
                                      test_size=test_size, random_state=1)
            elif self.resampling_strategy in ['cv', 'partial-cv',
                                              'partial-cv-iterative-fit']:
...
def eval_holdout(
    ...
    max_iter = instance.get('max_iter', 0)
    ...
    evaluator.fit_predict_and_loss(max_iter=max_iter)
...
def eval_partial_cv(
    ...
    evaluator.partial_fit_predict_and_loss(fold=fold, max_iter=max_iter)
