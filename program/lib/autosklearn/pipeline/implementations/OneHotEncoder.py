...
[1, 0, 2]])  # doctest: +ELLIPSIS
    ...
    def _fit_transform(self, X):
        if np.nanmax(X) > 10000:
            num_uniques = [len(np.unique(x)) for x in X.transpose()]
            self.max_num_uniques_ = np.max(num_uniques)
            X = X % (self.max_num_uniques_ * 5)
        else:
            self.max_num_uniques_ = None
    ...
    def _transform(self, X):
        ...
        if self.max_num_uniques_ is not None:
            X = X % (self.max_num_uniques_ * 5)
